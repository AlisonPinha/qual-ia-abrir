...
<script>
  ...
  async function reivindicar(payload, automatico = false) {
    ...
    try {
      ...
      if (automatico) {
        retorno.textContent = dados.indisponivel
          ? 'A conferência automática está temporariamente indisponível. Confirme abaixo os dados da compra.'
          : 'Não reconheci uma compra neste aparelho. Confirme abaixo o e-mail e o WhatsApp usados no checkout.';
      ...
      if (automatico) {
        retorno.textContent = 'Não consegui conferir este aparelho agora. Confirme abaixo os dados da compra.';
        retorno.hidden = false;
      } else {
