...
|---|---|---|---|---|
| 1.1 | **Loading + espelho das respostas** antes do resultado | É o item que os três citam no episódio e o único do fluxo deles que não existe aqui. Dá função às 10 perguntas que hoje somem sem deixar rastro | Claude | A penúltima tela repete as respostas da pessoa e só então revela o teaser |
| 1.2 | **`orcamento` passa a votar** e ganha a opção "Nada, quero só o que é grátis" | Hoje a faixa escolhida não muda nada, num produto que vende custo real. E o catálogo tem 4 ferramentas gratuitas que nunca são priorizadas | Claude | Duas pessoas com orçamentos opostos recebem stacks diferentes, medido no `testar_motor.mjs` |
| 1.3 | **Endurecer o `onde`** | Quem responde "no celular" recebe Lovable como "assina agora". O peso atual é de 1 ponto, insuficiente | Claude | Nenhuma combinação com celular devolve Lovable, Claude Code ou n8n na primeira camada |
| 1.4 | **Pular o que não faz sentido para iniciante** | Quem diz "nenhuma ferramenta" e "não pago nada" ainda recebe "já assinou alguma que não usou?" e "quantas vezes refaz o prompt?" | Claude | As duas perguntas somem quando `quantas` = "Nenhuma ainda" |
...
| 1.6 | **Reason why na abertura + escassez sob o botão** | "Você está respondendo porque a solução não é genérica" justifica cada pergunta seguinte. A escassez abaixo do botão é padrão da etapa 1 deles | Claude | Ambos no ar, sem alterar a simplicidade da etapa 1 |
| 1.7 | **`estilo` passa a votar** | Quem diz "prefiro dominar uma a fundo" recebe três ferramentas igual | Claude | A composição muda entre as duas respostas |
